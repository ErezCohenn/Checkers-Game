...
namespace EnglishCheckersLogic
{
    public class Game
    {
        ...
        private bool hasNoAvailableMoves(Player i_PlayerToCheck)
        {
            createPossibleMoves();

            return i_PlayerToCheck.EatingPossibleMoves.Count == 0 && i_PlayerToCheck.RegularPossibleMoves.Count == 0;
        }
        ...
        public bool MoveManager(Movement i_NextMove)
        {
            Pawn destinedPawnToMove = null;
            bool ateInLastMove = false;
            bool validMove = true;

            if (!isValidMove(i_NextMove))
            {
                validMove = false;
            }
            else
            {
                destinedPawnToMove = m_CurrentPlayer.SearchPawnByLocation(i_NextMove.CurrentPosition);
                executeMove(i_NextMove, destinedPawnToMove, ref ateInLastMove);
                if (m_EatingSequence)
                {
                    m_EatingSequence = false;
                }
        ...
        private void executeMove(Movement i_NextMoveToExecute, Pawn o_PawnToMove, ref bool o_AteInCurrentMove)
        {
            ...
            int diffrenceBetweenPositions = Math.Abs(directionToMove.Row);
            ...
            if (diffrenceBetweenPositions == 2)
            {
                ...
                removePawnFromBoard(eatenPawnLocation);
                o_AteInCurrentMove = true;
            }

            updatePawnOnBoard(o_PawnToMove, i_NextMoveToExecute);
        }

        private void updatePawnOnBoard(Pawn o_CurrentPawn, Movement i_NextMove)
        {
            m_Board.SetCellValue(i_NextMove.CurrentPosition, Pawn.eType.Empty);
            m_Board.SetCellValue(i_NextMove.NextPosition, o_CurrentPawn.Type);
            o_CurrentPawn.Location = i_NextMove.NextPosition;
        }

        private void removePawnFromBoard(Position i_EatenPawnLocation)
        {
            m_Board.SetCellValue(i_EatenPawnLocation, Pawn.eType.Empty);
            m_PreviousPlayer.RemovePawnFromPawnsArray(i_EatenPawnLocation);
        }

        private void setComputerNextMove()
        {
            int randomIndexInMovesArrForNextMove = 0;

            if (m_CurrentPlayer.EatingPossibleMoves.Count > 0)
            {
                randomIndexInMovesArrForNextMove = m_RandomNumber.Next(0, m_CurrentPlayer.EatingPossibleMoves.Count);
                m_ComputerMove = m_CurrentPlayer.EatingPossibleMoves[randomIndexInMovesArrForNextMove];
            }
            else
            {
                createPossibleMoves();
                randomIndexInMovesArrForNextMove = m_RandomNumber.Next(0, m_CurrentPlayer.RegularPossibleMoves.Count);
                m_ComputerMove = m_CurrentPlayer.RegularPossibleMoves[randomIndexInMovesArrForNextMove];
            }
        }
